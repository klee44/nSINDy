...
plt.plot(t,data['test_data'][target_id,:,0],lw=3,color='g')
plt.plot(t,test_sol[target_id,:,0],lw=2,color='tomato',ls='--')

plt.plot(t,data['test_data'][target_id,:,1],lw=3,color='r')
plt.plot(t,test_sol[target_id,:,1],lw=2,color='deepskyblue',ls='--')

plt.plot(t,data['test_data'][target_id,:,2],lw=3,color='b')
plt.plot(t,test_sol[target_id,:,2],lw=2,color='yellow',ls='--')

plt.margins(0,0.04)
plt.title('Hopf bifurcation')
#plt.tight_layout()

save_file = os.path.join(fig_save_path,"hopf_example.png")
plt.savefig(save_file)
plt.close(fig)
plt.close('all')
plt.clf()
