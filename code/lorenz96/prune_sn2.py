...
data = np.load("../data/lorenz96_torch.npz")
h_ref = 1e-2 
Time = 30.72 
...
val_data = train_data #torch.utils.data.DataLoader(torch.tensor(data['val_data']),batch_size=50)
...
#		scheduler.step()
	...
	if itr >= 1900:#29000:
		with torch.no_grad():
			...
			#for d in val_data:
			pred_y = odeint(odefunc, train_data[0,0,:], t, method=args.odeint).to(device).transpose(0,1)
			val_loss = torch.mean(torch.abs(pred_y - train_data[0,:,:])).item()
